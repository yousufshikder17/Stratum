...
def validated(*observations):
    return [
        GUARD.validate(o, manifest=MANIFEST, mode=IngestMode.BACKFILL) for o in observations
    ]
...
def test_pit_read_returns_latest_vintage_as_of(store):
    ...
    seen = list(store.read(signal_type="social.sentiment",
                           as_of=AsOf.at(datetime(2026, 4, 15, tzinfo=UTC))))
    assert len(seen) == 1
    ...
    assert list(store.read(signal_type="social.sentiment",
...
def test_conflicting_content_under_same_id_is_rejected(store):
    store.write(validated(FIRST), run_id="run-1")
    impostor = sentiment(FIRST.observation_id, "$AAA",
                         datetime(2026, 4, 3, tzinfo=UTC), "v-0403", 0.99)
...
def test_in_place_update_is_rejected(store):
    store.write(validated(FIRST), run_id="run-1")
    overwrite = sentiment("01J0000000000000000000009", "$AAA",
                          datetime(2026, 4, 4, tzinfo=UTC), "v-0403", 9.9)
...
def test_snapshot_is_content_addressed_and_as_of_scoped(store, tmp_path):
    store.write(validated(FIRST, REVISED), run_id="run-1")
    early = store.snapshot(as_of=AsOf.at(datetime(2026, 4, 15, tzinfo=UTC)),
                           target_dir=tmp_path / "snap-early")
    late = store.snapshot(as_of=AsOf.at(datetime(2026, 6, 1, tzinfo=UTC)),
                          target_dir=tmp_path / "snap-late")
